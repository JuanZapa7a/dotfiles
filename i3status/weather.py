...
def format_weather(obs, format_str):
    ...
    data['temp_c_min'] = round(weather.get_temperature(unit='celsius')['temp_min'])
    data['temp_c_max'] = round(weather.get_temperature(unit='celsius')['temp_max'])
    data['temp_k'] = round(weather.get_temperature(unit='kelvin')['temp'])
    data['text'] = weather.get_status()
    data['text_d'] = weather.get_detailed_status()
...
if __name__ == '__main__':
    ...
    p.add_argument('--format', metavar='F',
            default='{city}, {text_d},  {sunrise}  {sunset}  {temp_c}°C ({temp_c_min}/{temp_c_max})°C 煮 {wind_speed_ms}m/s{wind_direction_arrow}  {humidity}%  {pressure}hPa',
    ...
    p.add_argument('--zip-country', type=str, default='ES',
                   help='set country for zip code lookup (defaults to ES)')
    ...
    owm = OWM(API_key=args.api_key, language='es', version='2.5')
